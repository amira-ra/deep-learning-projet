...
def greedy_decode(model, trg_vocab, src, max_len=20, device='cpu'):
    """
    Décodage Glouton (Greedy Search).
    trg_vocab : vocabulaire de la langue CIBLE (utilisé pour <SOS>/<EOS> côté décodeur).
    """
    model.eval()
    src = src.to(device)
    ...
    with torch.no_grad():
        ...
        # Commencer avec <SOS> (vocabulaire cible)
        decoder_input = torch.tensor([[trg_vocab.token2idx['<SOS>']]]).to(device)
        decoded_tokens = []
...
def beam_search_decode(model, trg_vocab, src, k=3, max_len=20, device='cpu'):
    """
    ...
    with torch.no_grad():
        ...
        beams = [
            ([trg_vocab.token2idx['<SOS>']], 0.0, hidden)
        ]
        ...
        for _ in range(max_len):
            ...
            for seq, log_prob, hidden in beams:
                ...
                for i in range(k):
                    ...
                    if token == trg_vocab.token2idx['<EOS>']:
        ...
        # Ajouter les faisceaux non complétés (on ne garde que seq et log_prob,
        # comme pour les séquences déjà complétées, afin d'uniformiser les tuples)
        completed.extend([(seq, log_prob) for seq, log_prob, _ in beams])

        # Score normalisé par la longueur (cf. fiche de synthèse, section 11.5)
        # pour éviter de favoriser systématiquement les séquences courtes.
        def length_normalized_score(item, alpha=0.7):
            seq, log_prob = item
            return log_prob / (len(seq) ** alpha)

        # Retourner la meilleure séquence (sans <SOS> et <EOS>)
        best_seq = max(completed, key=length_normalized_score)[0]
        return best_seq[1:] if best_seq and best_seq[0] == trg_vocab.token2idx['<SOS>'] else best_seq
